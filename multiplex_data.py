"""Polars tables for a (issuer, term) multiplex NetworkX graph."""
...
def filter_tables(
    ...
    nodes_f = nodes.filter(pl.col("term").is_in(term_list)) if not nodes.is_empty() else nodes
    intra_f = intra.filter(pl.col("term").is_in(term_list)) if not intra.is_empty() else intra
    if inter.is_empty():
        inter_f = inter
    else:
